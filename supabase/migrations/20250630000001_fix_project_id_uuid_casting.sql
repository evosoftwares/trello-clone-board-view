-- Fix UUID casting issue in stored procedure
-- The issue is that project_id::uuid fails when project_id is null
...
BEGIN
    ...
    IF is_completion AND task_record.assignee IS NOT NULL AND task_record.function_points > 0 THEN
        -- Safe UUID casting for project_id (handle NULL case)
        safe_project_id := CASE 
            WHEN task_record.project_id IS NOT NULL THEN task_record.project_id::uuid
            ELSE NULL
        END;
